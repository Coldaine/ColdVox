...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match result {
                ...
                match device_event_rx.try_recv() {
                    Ok(event) => {
                        match event {
                            ...
                            DeviceEvent::DeviceSwitchFailed { attempted, fallback } => {
                                println!("❌ Device switch failed: attempted={}, fallback={:?}", attempted, fallback);
                            }
        ...
        Err(e) => {
            ...
            println!("Available device candidates: {:?}", monitor.get_device_candidates());
