...
#[cfg(feature = "mki")]
use mouse_keyboard_input::{VirtualKeyboard, VirtualDevice, KeyboardControllable, Key};
#[cfg(feature = "mki")]
...
impl MkiInjector {
    ...
    async fn type_text(&mut self, text: &str) -> Result<(), InjectionError> {
        ...
        let result = tokio::task::spawn_blocking(move || {
            ...
            keyboard.key_sequence(&text_clone).map_err(|e| InjectionError::MethodFailed(e.to_string()))?;
    ...
    async fn type_text(&mut self, _text: &str) -> Result<(), InjectionError> {
        Err(InjectionError::MethodUnavailable("MKI feature not enabled".to_string()))
    }
    ...
    async fn trigger_paste(&mut self) -> Result<(), InjectionError> {
        ...
        let result = tokio::task::spawn_blocking(|| {
            ...
            keyboard.key_sequence("ctrl+v").map_err(|e| InjectionError::MethodFailed(e.to_string()))?;
