        ...
        loop {
            ...
            if ctrl && meta {
                if !active {
                    ...
                    let _ = event_tx.blocking_send(VadEvent::SpeechStart { timestamp_ms: ts, energy_db: 0.0 });
                }
            } else if active {
                active = false;
                indicator.hide();
                ...
                let ts = app_start.elapsed().as_millis() as u64;
                let _ = event_tx.blocking_send(VadEvent::SpeechEnd { timestamp_ms: ts, duration_ms: duration, energy_db: 0.0 });
            }
