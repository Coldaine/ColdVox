...
use crate::text_injection::types::{
    InjectionConfig, InjectionError, InjectionMethod, InjectionMetrics,
};
...
impl AtspiInjector {
    ...
    async fn insert_text_direct(
        ...
        let editable_text = EditableText::new(accessible)
            .await
            .map_err(|e| InjectionError::Atspi(e))?;

        // Get current text length to insert at end
        let text_length = editable_text
            .get_text(0, -1)
            .await
            .map_err(|e| InjectionError::Atspi(e))?
            .len() as i32;
        ...
        editable_text
            .insert_text(text_length, text)
            .await
            .map_err(|e| InjectionError::Atspi(e))?;

        let duration = start.elapsed().as_millis() as u64;
        self.metrics
            .record_success(InjectionMethod::AtspiInsert, duration);
        info!(
            "Successfully inserted text via AT-SPI2 EditableText ({} chars)",
            text.len()
        );
...
impl super::types::TextInjector for AtspiInjector {
    ...
    async fn inject(&mut self, text: &str) -> Result<(), InjectionError> {
        ...
        let focus_status = self.focus_tracker.get_focus_status().await.map_err(|e| {
            let duration = start.elapsed().as_millis() as u64;
            self.metrics
                .record_failure(InjectionMethod::AtspiInsert, duration, e.to_string());
            e
        })?;
        ...
        if focus_status == FocusStatus::NonEditable {
            ...
            return Err(InjectionError::MethodUnavailable(
                "Focused element not editable".to_string(),
            ));
        }
        ...
        Err(InjectionError::MethodFailed(
            "AT-SPI2 injection failed".to_string(),
        ))
    }
