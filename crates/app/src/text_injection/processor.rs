...
impl InjectionProcessor {
    ...
    fn determine_use_paste(&self) -> bool {
        match self.config.injection_mode.as_str() {
            "paste" => true,
            "keystroke" => false,
            "auto" => self.session.buffer_preview().len() > self.config.paste_chunk_chars as usize,
            _ => self.session.buffer_preview().len() > self.config.paste_chunk_chars as usize,
        }
    }
}
...
impl AsyncInjectionProcessor {
    ...
    pub fn new(
        ...
        let injection_metrics = Arc::new(Mutex::new(
            crate::text_injection::types::InjectionMetrics::default(),
        ));

        // Create processor with shared metrics
        let processor = Arc::new(TokioMutex::new(InjectionProcessor::new(
            config.clone(),
            pipeline_metrics,
