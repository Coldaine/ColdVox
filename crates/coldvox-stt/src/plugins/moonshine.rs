...
mod tests {
    ...
    #[test]
    #[cfg(feature = "moonshine")]
    fn verify_sample_rate_ok_for_16khz() {
        // This test relies on SAMPLE_RATE_HZ being 16000.
        // If this constant changes, this test should fail.
        assert_eq!(SAMPLE_RATE_HZ, 16000, "Global sample rate must be 16kHz for Moonshine");
        let plugin = MoonshinePlugin::new();
        assert!(plugin.verify_sample_rate().is_ok());
    }

    #[tokio::test]
    #[cfg(feature = "moonshine")]
    async fn buffer_overflow_protection() {
        let mut plugin = MoonshinePlugin::new();
        plugin.initialized = true; // Mock initialization

        let large_chunk = vec![0i16; MAX_AUDIO_BUFFER_SAMPLES];
        let overflow_chunk = vec![0i16; 100];

        // Fill the buffer exactly to its limit
        let result = plugin.process_audio(&large_chunk).await;
        assert!(result.is_ok());
        assert_eq!(plugin.audio_buffer.len(), MAX_AUDIO_BUFFER_SAMPLES);

        // Try to add more samples, which should be rejected/truncated
        let result = plugin.process_audio(&overflow_chunk).await;
        assert!(result.is_ok());

        // The buffer size should not exceed the maximum
        assert_eq!(plugin.audio_buffer.len(), MAX_AUDIO_BUFFER_SAMPLES);
    }
}
