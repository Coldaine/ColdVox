...
use coldvox_audio::{AudioRingBuffer, AudioCaptureThread, DeviceMonitor};
...
mod device_hotplug_tests {
    ...
    fn test_device_monitor_basic_functionality() {
        ...
        assert!(!candidates.is_empty(), "Should have at least one device candidate");
    ...
    fn test_audio_capture_thread_with_device_events() {
        ...
        match result {
            ...
            Err(e) => {
                ...
                println!("Audio capture thread creation failed (expected in CI): {}", e);
            }
        }
    }
    ...
    fn test_device_event_types() {
        ...
        assert!(matches!(disconnected_event, DeviceEvent::CurrentDeviceDisconnected { .. }));
