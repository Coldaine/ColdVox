...
const FRAME_READER_BUFFER_SIZE: usize = 4096;
...
pub(crate) const DEFAULT_SAMPLE_RATE_HZ: u32 = 16_000;
pub(crate) const DEFAULT_FRAME_SIZE_SAMPLES: usize = 512;
...
impl ChunkerWorker {
    ...
    async fn run(&mut self, running: Arc<AtomicBool>) {
        ...
        while running.load(Ordering::SeqCst) {
            ...
            if let Some(frame) = self.frame_reader.read_frame(FRAME_READER_BUFFER_SIZE) {
    ...
    /// Extract a fixed number of samples from the front of the buffer
    fn extract_samples_from_buffer(&mut self, count: usize) -> Vec<i16> {
    ...
    fn process_frame(&mut self, frame: &super::capture::AudioFrame) -> Vec<i16> {
        ...
        let mono_samples = self.convert_to_mono(frame);
    ...
    fn convert_to_mono(&self, frame: &super::capture::AudioFrame) -> Vec<i16> {
        if frame.channels == 1 {
            frame.samples.clone()
        } else {
