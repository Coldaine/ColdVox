    ...
    fn test_candidate_order_default_first() {
        ...
        for candidate in &candidates {
            if let Some(_device) = manager.find_device_by_name(candidate) {
                let info = manager.enumerate_devices().into_iter().find(|d| d.name == *candidate).unwrap();
                assert!(!info.supported_configs.is_empty(), "Candidates should have viable configs");
            }
        }
    }
